...
class JointstatePublisher(Node):
    ...
    def sim_loop(self):
        ...
        if self.teleop_on:
            ...
            m = round(np.sqrt(np.linalg.det(j_reduce @ j_reduce.transpose())),4)
            ...
            if m > 1 * 10**-26 :
                ...
                qd = [0.0 ,0.0 ,0.0]
                self.get_logger().info(f'Singularity Robot Stop')
            
            self.get_logger().info(f'qd : {qd}')

            self.q[0] = self.q[0] + (qd[0] * self.dt)
            self.q[1] = self.q[1] + (qd[1] * self.dt)
            self.q[2] = self.q[2] + (qd[2] * self.dt)
    ...
    def teleop_keyboard_callback(self ,msg:Twist):
        if self.teleop_on:
            self.cmd_vel[0] = msg.linear.x * 1000
            self.cmd_vel[1] = msg.linear.y * 1000
            self.cmd_vel[2] = msg.linear.z * 1000
